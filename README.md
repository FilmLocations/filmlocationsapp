...
* User can view a map displaying nearby movies based on his current location
    * on the bottom of the screen, a details list containing the movies that are pined on the map is displayed
    * each item in that list contains: movie’s poster, title, release year, address for that location as long as the distance from the current position to that point
